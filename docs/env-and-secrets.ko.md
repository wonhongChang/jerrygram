...
## Stream Processing 설정

`SearchTrendStreamProcessor`는 Kafka `search-events`를 consume하고 Redis 검색 트렌드 bucket을 갱신합니다. 로컬에서 restart할 때 Kafka offset을 재사용하려면 consumer group을 안정적으로 유지하세요.
